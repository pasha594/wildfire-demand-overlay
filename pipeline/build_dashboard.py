...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .mtab.score th.why, .mtab.score td.why { padding-left: 22px; }
...
<div class="wrap">
  ...
  <section class="overview" id="overview" hidden>
    ...
    <div class="ocols">
      <div class="ocol"><h3>Search demand vs our clicks, by state</h3><div id="oscore"></div></div>
  ...
  <footer class="notes">
    ...
    than searching fell (impressions ÷ search below 0.6 — a ranking or indexing gap); "shown, not clicked" = we were
    shown but clicks fell behind impressions (clicks ÷ impressions below 0.6 — a title/snippet or position gap);
    "Google clicks kept pace" = our Google clicks rose with searching, so the shortfall is in other engines or
    tracking; otherwise "visibility and clicks both slipped".
    ...
    <p><b>Metro view.</b> The dropdown on a state card narrows the chart to one metro area (fire diamonds too: only fires
    that started inside the metro's counties in that state): site traffic counts only
...
function modeStatsHtml(st, sel) {
  ...
  if (vf.length)
    h += `<span><b>${vf.length}</b> <span class="lbl">fires${sel ? " in this metro" : ""} ·</span> <b class="fscount">${vf.filter(isImpactful).length}</b> <span class="lbl">impactful</span></span>`;
  else if (sel && chartFires(st).length)
    h += `<span class="lbl">no fires over 100 acres in this metro</span>`;
...
function whyOf(h) {
  ...
  if (g.I / g.Dg < 0.6)
    return `losing visibility <span class="lbl">· Google showed us ${liftFmt(g.I)} as often as usual while searches ran ${liftFmt(g.Dg)} usual</span>`;
  if (g.C != null && g.I > 0 && g.C / g.I < 0.6)
    return `shown, not clicked <span class="lbl">· shown ${liftFmt(g.I)} as often as usual but clicked ${liftFmt(g.C)} as often</span>`;
  if (g.C != null && h.D != null && g.C >= 0.8 * h.D)
    return `Google clicks kept pace <span class="lbl">· clicks ${liftFmt(g.C)} usual, in line with searches; the gap is outside Google search</span>`;
  return `visibility and clicks both slipped <span class="lbl">· shown ${liftFmt(g.I)} as often as usual, clicked ${liftFmt(g.C)} as often</span>`;
}
...
function buildOverview() {
  ...
  document.getElementById("osub").textContent =
    `last ${WIN} days (to ${fdate(DATA.dates[end])}) vs each state's typical day · clicks = organic visits from search engines`
  ...
  const gscRead = h => !h.g || h.g.low || h.g.I == null ? `<span class="lbl">no Search Console signal</span>`
    : `<span class="lbl">shown ${liftFmt(h.g.I)} as often as usual${h.g.C != null ? `, clicked ${liftFmt(h.g.C)} as often` : ""}</span>`;
  ...
  let html = rows.length
    ...
    : `<div class="mempty">No state has enough search and traffic over the last ${WIN} days to judge.</div>`;
